---
name: migration-plan
description: DBスキーマ変更・大規模データ移行の計画を作成する。後方互換マイグレーション(Expand-Migrate-Contractパターン)、オンラインDDL、ロック影響、ロールバック手順、ゼロダウンタイム戦略を含む。ユーザーが「マイグレーション計画」「スキーマ変更」「カラム追加/削除」「データ移行」「ゼロダウンタイム」と言ったとき、DBスキーマ変更が必要な文脈で、`/migration-plan` と打ったときには必ずこのskillを使う。本番DBの変更は失敗コストが極めて高いので、関連キーワードがあれば必ず発動すること。
---
