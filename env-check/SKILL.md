---
name: env-check
description: 環境変数の設定漏れ・本番/開発の差分検出、`.env.example` との同期、シークレットのコード混入チェックを行う。ユーザーが「環境変数チェック」「本番と開発の差分」「.env漏れない?」「設定が反映されない」「シークレット見て」と言ったとき、デプロイ前/環境問題の文脈で、`/env-check` と打ったときには必ずこのskillを使う。
---
